...
start_date = housing.Date.dt.date.max()
end_date = datetime.date(housing.Date.dt.year.max(), 12, 1)
if (end_date.year - start_date.year) * 12 + (end_date.month - start_date.month) <=6:
    end_date = datetime.date(housing.Date.dt.year.max()+1, 12, 1)
    max_prediction_length = (end_date.year - start_date.year) * 12 + (end_date.month - start_date.month)
else:
    end_date = datetime.date(housing.Date.dt.year.max(), 12, 1)
    max_prediction_length = (end_date.year - start_date.year) * 12 + (end_date.month - start_date.month)
print(end_date.year)
    ...
    html.Div([
        ...
        dcc.Tab(label='Forecasts', value='tab-3', children=[
            html.Div([
                ...
                html.Label(f"Forecast of {end_date.year}", style={'width': '100%', 'display': 'flex', 'align-items': 'center','font-weight':'bold','justify-content':'center'}),
            dash_table.DataTable(id='datatable-interactivity-forecast',
                    columns=[
                        {"name": "Municipality", "id": "Municipality", "type": "text"},
                        {"name": "Community", "id": "Geography", "type": "text"},
                        {"name": "Home Type", "id": "HomeType", "type": "text"},
                        {"name": f"Median Price of {start_date.year}","id": "Median Price of Year", "type": "numeric", "format": FormatTemplate.money(2)},
                        {"name": f"Median Price of {end_date.year}", "id": f"Median Price of {end_date.year}", "type": "numeric", "format": FormatTemplate.money(2)},
                        {"name": "YoY Price (%)", "id": "Price YoY", "type": "numeric",'format': FormatTemplate.percentage(2)}
                    ],
